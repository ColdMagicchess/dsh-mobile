...
- **1.9.2 没有 DrawScope.drawLayer/drawContext 扩展（1.0.4 编译坑）**：想给面板挖侵蚀洞时 `import androidx.compose.ui.graphics.drawscope.drawLayer/drawContext` 均 Unresolved——改用原生通路：`scope.drawContext.canvas.nativeCanvas` + `canvas.saveLayer(...)` + `PorterDuff.Mode.DST_OUT` 软边 RadialGradient 圆，零版本依赖且语义完全一致。教训：Compose 图形 API 跨版本挪动频繁，遮罩合成这种底层操作直接走 nativeCanvas 最稳。
- **粒子带别缩尺寸（1.0.4 观感坑）**：消散粒子若随寿命缩小，满铺网格出现缝隙 → 整片看起来是"离散方块马赛克"（用户反馈"很模糊"）。保持 sz=step 满铺 + 只降 alpha，粒子带就是连续绸缎；面板本体永远 drawBitmap 清晰像素，粒子只负责侵蚀边界。
