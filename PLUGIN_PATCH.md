...
> **0.3.12 起本补丁整体作废（2026-09-02；0.3.15 仍沿用 `/remote` 通道）**：dsh-web-all 0.3.12 移除了 `/m/api` BFF，
...
> **当前状态（2026-09-05，源码安装部署 0.3.15）**：本机已从**源码构建安装** 0.3.15 全家桶
> （`plugin-src/dsh-web-0.3.15`，经 `link-profile.mjs` junction 进 profile；0.3.12 源码仍保留于
> `plugin-src/dsh-web-0.3.12` 可作回滚）。App 手机通道仍走 remote-web-ui 的 **`/remote` 门控镜像**
> （自 0.3.12 起的机制，0.3.15 延续不变）：归档走宿主 RPC `workspace/archiveSession`，
> 归档名单走 dsh-session-archive 的 `inventory`——**仍无需任何运行时补丁**。下文的运行时补丁
> 流程仅适用于**从 npm 安装的官方 0.3.6 插件**（0.3.9+ 已无 `/m/api`，运行时补丁无从下手，需要走源码级方案）。
